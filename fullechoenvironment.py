...
class ReinforcementNetworkEnvironment(gym.Env):
    ...
    def environmentreset(self):
        ...
        self.ifcomplete = False
        self.queuestimatedtime = [self.timebetweenqueues]*self.number_of_nodes
    ...
    def fullecho(self, action):
        present_event = self.present_event
        current_time = self.present_event.estimatedtime
        current_node = self.present_event.node
        ...
        if action < 0 or action not in self.linkmap[current_node]:
            return reward, (current_node, present_event.dest)
            ...
            return reward, (next_node, present_event.dest)
